...
public class CIBlocksImpl {
	@ApiStatus.Internal
	public static <MF> MF customChairModelFile(Object o, String parent, String name,
	...
	@ApiStatus.Internal
	public static <MF> MF getExistingModelFile(Object o, String name) {
		BlockStateProvider p = (BlockStateProvider) o;
		return (MF) p.models().getExistingFile(p.modLoc(name));
	}

	@ApiStatus.Internal
	public static <MF> MF createModelFileWithExistingParent(Object p, String parent, String name) {
		BlockStateProvider provider = (BlockStateProvider) p;
		return (MF) provider.models().withExistingParent(name, provider.modLoc(parent));
	}
	...
	public static void simpleBlock(DataGenContext<Block, ?> c, RegistrateBlockstateProvider p, ResourceLocation texture) {
		BlockStateProvider provider = (BlockStateProvider) p;
		provider.simpleBlock(c.get(), provider.models().cubeAll(c.getName(), texture));
	}
}
